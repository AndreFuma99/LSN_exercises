...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising
    double delta_E, p, r;
    delta_E = _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H;
    p = 1.0/(1.0+exp(-2.0*_beta*delta_E));
    r = _rnd.Rannyu();
    if(r<p){
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        ofstream coutgr("../OUTPUT/gofr_blocks.dat");   // file in cui stampare g(r) in funzione della distanza radiale r
        coutgr.close();
        
        ofstream coutrg("../OUTPUT/gofr_ave.dat");   // file in cui stampare i risultati del data blocking
...
void System :: measure(){ // Measure properties
  ...
  vec counts(_n_bins);
  distance.resize(_ndim);
  for (int k=0; k<_n_bins; k++) counts[k] = 0.;
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        distance(2) = this->pbc( _particle(i).getposition(2,true) - _particle(j).getposition(2,true), 2);
        dr = sqrt( dot(distance,distance) );
        if(dr < _halfside[0]){   // per riempire l'istogramma
          bin = dr/_bin_size;
          _measurement(_index_gofr + bin) += 2;   // incremento il bin di 2 ogni volta che trovo due particelle ad una distanza tra r e r+dr
        }
        if(dr < _r_cut){
  ...
  if (_measure_pressure and _measure_kenergy){
    pressure_temp = _ptail*(_npart/_volume) + _rho*((2.0/3.0) * kenergy_temp) + (16.0/_volume)*pressure_temp;
  ...
  if (_measure_magnet){ 
    ...
    magnetization /= double(_npart);   // Da usare solo quando H non-nullo
  ...
  if (_measure_cv and _measure_tenergy){ 
    ...
    for (int i=0; i<_npart; i++){
      s_i = double(_particle(i).getspin());
      s_j = double(_particle(this->pbc(i+1)).getspin());
      sh += _J*_J*s_i*s_i*s_j*s_j + ((_H*_H)/4.)*(s_i+s_j)*(s_i+s_j) + _J*_H*s_i*s_j*(s_i+s_j);
    }
    sh = (_beta*_beta*(sh-tenergy_temp*tenergy_temp*_npart))/_npart;
    _measurement(_index_cv) = sh;
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi and _measure_magnet){ 
    susceptibility = (_beta*magnetization*magnetization)/_npart;
