...
void System :: measure(){ // Measure properties
  ...
  if (_measure_pressure and _measure_kenergy){
    pressure_temp = _ptail + _rho*((2.0/3.0) * kenergy_temp) + (16.0/_volume)*pressure_temp;
  ...
  if (_measure_cv){ 
    ...
    for (int i=0; i<_npart; i++){
      s_i = double(_particle(i).getspin());
      s_j = double(_particle(this->pbc(i+1)).getspin());
      sh += - _J * s_i * s_j - 0.5 * _H * (s_i + s_j);
    }
    _measurement(_index_cv) = sh*sh;
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    double s_i;
    ...
    _measurement(_index_chi) = susceptibility*susceptibility;
  }
