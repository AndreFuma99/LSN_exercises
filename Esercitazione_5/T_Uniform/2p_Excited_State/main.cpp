...
// Ora utilizzo l'algoritmo di Metropolis per campionare il modulo quadro della funzione d'onda relativa ad uno dei tre stati 2p dell'atomo di H utilizzando una probabilità di transizione uniforme   
   ...
   double x_n = sqrt(10.0);   // ascissa del punto di partenza
   double y_n = sqrt(10.0);   // ordinata del punto di partenza
   double z_n = sqrt(10.0);   // altezza del punto di partenza
   double step = 2.98;   // step della probabilità di transizione uniforme T(x|y)
   
   
   // Prima di tutto devo trovare uno step della probabilità di transizione uniforme T(x|y) : la probabilità di accettazione A è circa = 0.5;
   /*
   double SUM = 0;
   ...
   for(int k=0; k<dim; k++){
      ...
      double x_p = x_n + p;
      double q = rnd.Rannyu(-step,step);
      double y_p = y_n + q;
      double r = rnd.Rannyu(-step,step);
      double z_p = z_n + r;
      	 
      double A = min(1., psi_210(x_p, y_p, z_p)/psi_210(x_n, y_n, z_n));
   ...
   */
   ...
   ofstream output_1;
   output_1.open("Es05_E1.csv");
   if (output_1.is_open()){
      output_1 << "Radius" << endl;
      for(int i=0; i<dim; i++){
         output_1 << R[i] << endl;
      }
   } else cerr << "PROBLEM: Unable to open Es05_E1.csv" << endl;
   output_1.close();
   ...
   coordinates.open("Es05_U1_xyz.csv");
   if (coordinates.is_open()){
      coordinates << "X" << "," << "Y" << "," << "Z" << endl;
      for(int i=0; i<M; i++){
         coordinates << X[i] << "," << Y[i] << "," << Z[i] << endl;
      }
   } else cerr << "PROBLEM: Unable to open Es05_U1_xyz.csv" << endl;
   coordinates.close();
