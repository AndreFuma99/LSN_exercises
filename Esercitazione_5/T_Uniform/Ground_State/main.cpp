...
// Ora utilizzo l'algoritmo di Metropolis per campionare il modulo quadro della funzione d'onda relativa allo stato fondamentale dell'atomo di H utilizzando una probabilità di transizione uniforme
   ...
   double step = 1.227;   // step della probabilità di transizione uniforme T(x|y)
   
   
   // Prima di tutto devo trovare uno step di T(x|y) : la probabilità di accettazione A è circa = 0.5;
   /*
   double SUM = 0;
   ...
   */
   ...
   coordinates_1.open("Es05_U0_xyz.csv");
   if (coordinates_1.is_open()){
      coordinates_1 << "X" << "," << "Y" << "," << "Z" << endl;
      for(int i=0; i<M; i++){
         coordinates_1 << X[i] << "," << Y[i] << "," << Z[i] << endl;
      }
   } else cerr << "PROBLEM: Unable to open Es05_U0_xyz.csv" << endl;
   coordinates_1.close();
