...
// Ora utilizzo l'algoritmo di Metropolis per campionare il modulo quadro della funzione d'onda relativa ad uno dei tre stati 2p dell'atomo di H utilizzando una probabilità di transizione gaussiana   
   ...
   double step = 1.88;   // step della probabilità di transizione gaussiana T(x|y)
   ...
   */
   ...
   coordinates.open("Es05_N1_xyz.csv");
   if (coordinates.is_open()){
      coordinates << "X" << "," << "Y" << "," << "Z" << endl;
      for(int i=0; i<M; i++){
         coordinates << X[i] << "," << Y[i] << "," << Z[i] << endl;
      }
   } else cerr << "PROBLEM: Unable to open Es05_N1_xyz.csv" << endl;
   coordinates.close();
