...
// Ora voglio calcolare al tempo t=0 attraverso Monte Carlo i prezzi europei Call-option e Put-option campionando il cammino discretizzato del prezzo dell'asset per un GBM
   ...
   for(int i=0; i<N; i++){   // per ogni blocco
      ...
      for(int j=0; j<L; j++){   // per ogni lancio (all'interno di un blocco)
      	 double S = S_0;   // S rappresenta il prezzo dell'asset
      	 for(int k=0; k<100; k++){
